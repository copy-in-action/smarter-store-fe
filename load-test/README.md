...
이 디렉토리는 k6를 사용한 예매 시스템의 부하 테스트 스크립트를 포함합니다.
...
# 환경 변수와 함께 실행
k6 run -e SCHEDULE_ID=123 -e TOTAL_USERS=50 booking-load-test.js
```
...
```bash
# 인증 토큰과 함께 실행
k6 run -e AUTH_TOKEN="your_token_here" -e SCHEDULE_ID=123 -e SEAT_ID=1 simple-booking-test.js

# 환경 변수 파일 사용
k6 run --env-file .env simple-booking-test.js
```
...
```bash
k6 run -e TOTAL_USERS=100 booking-load-test.js
```
...
```bash
k6 run -e USERS=100 -e AUTH_TOKEN="your_token" simple-booking-test.js
```
...
**해결**:
- 백엔드 팀에 테스트 계정 생성 요청
- 또는 `simple-booking-test.js`를 사용하여 단일 계정으로 테스트
