/**
 * Generated by orval v7.17.0 🍺
 * Do not edit manually.
 * Smarter Store API
 * Smarter Store 백엔드 API 문서

**관리자 대시보드**: [매출 현황 대시보드](/admin/dashboard.html)
 * OpenAPI spec version: 1.0.0
 */

/**
 * 액션
 */
export type AuditLogDetailResponseAction = typeof AuditLogDetailResponseAction[keyof typeof AuditLogDetailResponseAction];


// eslint-disable-next-line @typescript-eslint/no-redeclare
export const AuditLogDetailResponseAction = {
  LOGIN: 'LOGIN',
  LOGIN_FAILED: 'LOGIN_FAILED',
  LOGOUT: 'LOGOUT',
  TOKEN_REFRESH: 'TOKEN_REFRESH',
  SIGNUP: 'SIGNUP',
  PASSWORD_CHANGE: 'PASSWORD_CHANGE',
  PROFILE_UPDATE: 'PROFILE_UPDATE',
  ACCOUNT_DELETE: 'ACCOUNT_DELETE',
  EMAIL_VERIFY: 'EMAIL_VERIFY',
  BOOKING_START: 'BOOKING_START',
  BOOKING_CONFIRM: 'BOOKING_CONFIRM',
  BOOKING_CANCEL: 'BOOKING_CANCEL',
  PAYMENT_REQUEST: 'PAYMENT_REQUEST',
  PAYMENT_COMPLETE: 'PAYMENT_COMPLETE',
  PAYMENT_FAIL: 'PAYMENT_FAIL',
  REFUND_REQUEST: 'REFUND_REQUEST',
  REFUND_COMPLETE: 'REFUND_COMPLETE',
  PERFORMANCE_CREATE: 'PERFORMANCE_CREATE',
  PERFORMANCE_UPDATE: 'PERFORMANCE_UPDATE',
  PERFORMANCE_DELETE: 'PERFORMANCE_DELETE',
  SCHEDULE_CREATE: 'SCHEDULE_CREATE',
  SCHEDULE_UPDATE: 'SCHEDULE_UPDATE',
  SCHEDULE_DELETE: 'SCHEDULE_DELETE',
  VENUE_CREATE: 'VENUE_CREATE',
  VENUE_UPDATE: 'VENUE_UPDATE',
  VENUE_DELETE: 'VENUE_DELETE',
  VENUE_SEATING_CHART_UPDATE: 'VENUE_SEATING_CHART_UPDATE',
  NOTICE_CREATE: 'NOTICE_CREATE',
  NOTICE_UPDATE: 'NOTICE_UPDATE',
  NOTICE_DELETE: 'NOTICE_DELETE',
  SEAT_HOLD: 'SEAT_HOLD',
  SEAT_RELEASE: 'SEAT_RELEASE',
} as const;
